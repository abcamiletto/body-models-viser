...
test("applyPose rejects skeleton and mesh bone length mismatches", () => {
  const bones: ViserBoneHandle[] = [{ wxyz: [1, 0, 0, 0], position: [0, 0, 0] }];
  const scene: BodyModelScene = {
    addFrame(name): ViserFrameHandle {
      return {
        name,
        wxyz: [1, 0, 0, 0],
        position: [0, 0, 0],
  ...
  let skeleton = [transform(0)];
  const model = createBodyModel<DemoParams>({
    modelName: "Demo",
    faces: [[0, 1, 2, 3]],
    skinWeights: [[1]],
    poseParameterNames: ["body_pose"],
    getRestPose: () => ({
      shape: [0],
      body_pose: [0],
      global_translation: [0, 0, 0],
    }),
    forward: () => ({ skeleton, mesh: [[0, 0, 0]] }),
  });
  ...
  assert.throws(
    ...
    /produced 0 skeleton joints for 1 mesh bones/,
  );
});
